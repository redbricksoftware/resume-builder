...
- Use `LevelFormat.BULLET` for bullet points (never unicode bullets)
- Use `TabStopType.RIGHT` with `TabStopPosition.MAX` for right-aligned dates on job title lines
- Validate with `python scripts/office/validate.py` if available

Using **python-docx** (`pip3 install python-docx`):
- Create a clean, ATS-friendly single-column layout
- Use `Pt(10)` body text, `Pt(14)` section headers, `Pt(18)` name
- Set narrow margins: `Inches(0.5)` all sides for 2-page fit
- Add horizontal rules between sections using paragraph borders
